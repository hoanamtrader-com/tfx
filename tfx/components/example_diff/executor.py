...
class _IncludedSplitPairs(object):
  ...
  def __init__(self, include_split_pairs: List[Tuple[str, str]]):
    if include_split_pairs is None:
      self._include_split_pairs = None
    else:
      self._include_split_pairs = set()
      for test_split, base_split in include_split_pairs:
        self._include_split_pairs.add((test_split, base_split))
...
class Executor(base_beam_executor.BaseBeamExecutor):
  ...
  def Do(self, input_dict: Dict[str, List[types.Artifact]],
    ...
    Args:
      input_dict: Input dict from input key to a list of Artifacts. TODO
      output_dict: Output dict from output key to a list of Artifacts. TODO
      exec_properties: A dict of execution properties. TODO
    ...
    split_pairs = sorted(split_pairs)
    ...
    with self._make_beam_pipeline() as p:
      for test_split, base_split in split_pairs:
        ...
        split = '%s_%s' % (test_split, base_split)
        logging.info('Processing split pair %s')
        ...
        def _iteration(p):
          ...
          output_uri = artifact_utils.get_split_uri([example_diff_artifact],
                                                    split)
          _ = (
              skew_stats
        ...
        _ = p | 'ProcessSplits[%s]' % split >> _iteration()  # pylint: disable=no-value-for-parameter
